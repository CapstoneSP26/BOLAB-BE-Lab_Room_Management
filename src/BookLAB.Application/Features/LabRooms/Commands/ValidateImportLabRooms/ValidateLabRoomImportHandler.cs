...
{
    public class ValidateLabRoomImportHandler : IRequestHandler<ValidateLabRoomImportQuery, ImportValidationResult<LabRoomImportDto>>
    {
        ...
        public async Task<ImportValidationResult<LabRoomImportDto>> Handle(ValidateLabRoomImportQuery request, CancellationToken cancellationToken)
        {
            var response = new ImportValidationResult<LabRoomImportDto>();
            ...
            for (var i = 0; i < request.LabRooms.Count; i++)
            {
                ...
                var rowResult = new RowResult<LabRoomImportDto>
                {
                    RowNumber = i + 2,
                    Data = item
                };
